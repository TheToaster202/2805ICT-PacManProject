...
public:
	...
	std::pair<int, int> AIPackage(TileMap* map, int const& currentX, int const& currentY, int const& targetX, int const& targetY, int const & ghostType, int const & ghostDirection, int const & mode);	//! Method that will decide which AI package to run and will return the next postion

private:

	void Blinky(TileMap* map, int const & targetX, int const & targetY, int const & gDir, int const& mode);		//! Blinky's movement algorithm
	void Pinky(TileMap * map, int const& targetX, int const& targetY, int const & gDir, int const& mode);		//! Pinky's movement algorithm
	void Inky(TileMap * map, int const& targetX, int const& targetY, int const & gDir, int const& mode);		//! Inky's movement algorithm
